...
export const api = {
  health: () => request<{
    status: string; items: number; rounds: number; example?: boolean
    /** True only for a tailnet peer (or a server run with ARBITER_OPEN=1). */
    authed?: boolean
  }>('/api/health'),
  ...
  importStatus: () => request<{
    ...
    mediastack?: Record<string, boolean>   // pre-rename servers
  }>('/api/import/status'),
