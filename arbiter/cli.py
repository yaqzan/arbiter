"""Command line — the only intended way to run the stack.
    ...
    python -m arbiter init                 # create YOUR library (empty)
    python -m arbiter init --examples      # ...or start from the example titles
    python -m arbiter serve                # API + built SPA on :5002
    python -m arbiter import-plex          # pull the watched corpus out of Plex
    python -m arbiter import-plex --dry-run
    python -m arbiter import-obsidian --type movie --dry-run
    python -m arbiter search "the wire"
    python -m arbiter add plex://show/<rating-key> --type anime
    python -m arbiter stats
    python -m arbiter top movie --limit 20
    python -m arbiter refit [--type movie]
    python -m arbiter purge-history <item_id> [--apply]
"""
...
from . import catalog, config, db, example, media_types, ranking, scorer
...
def cmd_serve(args):
    ...
    if db.using_example():
        print(f'  ! no library at {config.DB_PATH} yet: serving the read-only example.')
        print('    Create yours with `python -m arbiter init`, then restart serve.')
...
def cmd_import_plex(args):
    ...
    if not config.plex_token():
        print('! No Plex token (sign in to Plex, or set ARBITER_PLEX_TOKEN)',
              file=sys.stderr)
        return 1
...
def cmd_import_obsidian(args):
    ...
    if read['overridden']:
        print(f'  hand-resolved    {read["overridden"]}  ({args.overrides or config.OBSIDIAN_OVERRIDES})')
...
def cmd_stats(args):
    ...
    print(f'  db: {config.DB_PATH}'
          + ('  (not created yet: showing the example library)' if db.using_example() else ''))
    last = db.get_state('last_plex_import')
    print(f'  last plex import: {_fmt_secs(last)}')
    print(f'  radarr/sonarr: ' + ', '.join(
        f'{k}={"up" if v else "down"}' for k, v in arr.available().items()))
    print()
...
def cmd_init(args):
    try:
        path = db.init_database()
    except FileExistsError as exc:
        print(f'! {exc}', file=sys.stderr)
        return 1
    if args.examples:
        count = example.seed()
        print(f'  created {path} with the {count} example titles')
    else:
        print(f'  created an empty library at {path}')
    print('  next: import-plex, import-obsidian, or add titles from the Add page')
    return 0


def main(argv=None):
    parser = argparse.ArgumentParser(prog='arbiter', description=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = parser.add_subparsers(dest='command', required=True)

    p = sub.add_parser('init', help='create your own library (refuses if one exists)')
    p.add_argument('--examples', action='store_true',
                   help='seed it with the example titles instead of starting empty')
    ...
    p.add_argument('--path', help='the vault media folder (default: ARBITER_OBSIDIAN_VAULT)')
    p.add_argument('--overrides', help='hand-resolved titles (default: '
                                       'data/obsidian-overrides.json)')
