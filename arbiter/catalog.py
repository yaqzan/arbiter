...
# a nature series and a feature documentary are both legitimate answers.
_EXPECTED_KIND = {
...
def _distinct_films(hits):
    ...
    Discover's index carries duplicate records — the same film twice, and
...
def resolve_obsidian_entry(entry, search=None):
    ...
    shared by several films its top hit is often not the famous one, and *a
    ...
    # 20, not 8: a one-word title can be a common enough word that the film itself
