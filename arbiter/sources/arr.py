"""Radarr + Sonarr — "do we actually have this on disk?"

Strictly an **enrichment**, never a source of truth, and never a hard dependency.
It is off unless `ARBITER_RADARR_URL`/`_KEY` (and/or the Sonarr pair) are set;
the *arrs can run on this machine or any other one the server can reach. When
configured but unreachable, every call fails soft and returns an empty index
rather than raising. An Arbiter page must render identically either way.
...
# in parallel (don't collapse this back into a serial loop at _TIMEOUT).** The
# *arrs are often unreachable, and on Windows a refused connect to a dead
# port costs seconds rather than failing instantly — probing them serially
...
def available(force=False):
    """Which of Radarr/Sonarr are reachable. Cached for `_PROBE_TTL`.

    An unconfigured half (no URL or no key) is reported down without a probe.
    """
    with _probe_lock:
...
def annotate(items):
    """Attach an `ownership` key to serialized items. No-op when neither *arr is reachable."""
