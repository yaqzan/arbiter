"""An Obsidian media log: a second watch ledger, often older than Plex.

The folder named by `ARBITER_OBSIDIAN_VAULT` (or `--path`) holds one note per
thing consumed (the original was exported out of Notion), with the interesting
facts in the frontmatter:
...
# and Arbiter has no contest for. An unmapped type is skipped, never guessed at.
TYPE_MAP = {
...
def normalize_title(title):
    ...
    "Dr Strangelove – Or How I Learned to Stop Worrying" has to meet
    "Dr. Strangelove: Or How I Learned to Stop Worrying" somewhere. Accents, case,
...
def clean_title(name):
    ...
    tell two films apart — `Solaris (1972)`, `The Thing (1982)` — which is the
    strongest signal in the whole import and has to come out of the string before
    it is searched, because Discover ranks the parenthesised form *below* a newer
    film of the same name. The rest is a `Media - ` prefix the Notion export left on
    a handful of notes.
    """
...
# `Some Show S01 (rewatch)`, `Some Show S04`, `Some Show The Final Season
# Part 3`. Anchored at the end and applied repeatedly, so a title that
# merely contains a number (`Apollo 13`, `2 Fast 2 Furious`) is
...
def series_title(name):
    """`The Wire S04` -> `The Wire`. Applied until it stops changing.

    The dangling-separator strip is why `The Wire S05 -` resolves: the note
...
def read_overrides(path=None):
    ...
    where the two differ: Discover can find a film only under its original
    title and still return a record with the English one.
    ...
    *before* season notes are grouped, an override's `title` also merges: it is
    what folds a `Some Show - Some Arc` note into its parent series.

    `manual` builds the row from the override itself, with no Plex record behind
    it. Discover's index simply does not contain some well-known films — no
    query reaches them — and a watched film with a
    ...
    resolved from a bare name: remakes share names, short one-word titles sink
    below twenty hits of shows that merely contain the word, and notes have typos.
    ...
    """
    file = Path(path or config.OBSIDIAN_OVERRIDES)
    if not file.exists():
        return {}
...
def read_vault(path=None, media_type=None, status='Finished', overrides=None):
    ...
    """
    if not (path or config.OBSIDIAN_VAULT):
        raise FileNotFoundError(
            'No Obsidian media folder configured: pass --path or set '
            'ARBITER_OBSIDIAN_VAULT (in .env or the environment)')
    folder = Path(path or config.OBSIDIAN_VAULT)
    ...
    for note in sorted(folder.glob('*.md')):
        ...
        # `Some Show S01..S06` is SIX notes about ONE thing this app ranks:
        # the TV contest judges "the show as a whole, not one season". Rewatches
        # collapse the same way. Films keep their own names — `Three Colours
        # I/II/III` are three films, not three seasons.
        ...
        # grouping keys on: that is what folds a `Some Show - Some Arc` note
        # into the series it is a season of, and averages their ratings.
