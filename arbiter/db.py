"""SQLite connection + schema, and the example-library fallback.

`data/arbiter.db` is YOUR library and is gitignored. Until it exists (`python -m
arbiter init` creates it), every read is served from a throwaway copy of the
example library in `examples/library.json`, built fresh in the temp folder once
per process, so a fresh clone can `serve` and look around straight away. Every
write path calls `require_own_data()` first, so nothing is ever recorded into
the example.

SQLite rather than a database server, because this catalog is
...
class NoLibrary(RuntimeError):
    """A write was attempted while reads are coming from the example library."""


def using_example():
    return not Path(config.DB_PATH).exists()


def require_own_data():
    if using_example():
        raise NoLibrary(
            f'no library at {config.DB_PATH} yet, so this is the read-only example. '
            'Run `python -m arbiter init` first (then restart `serve`).')


_example_lock = threading.RLock()
_example_path = None


def _example_db():
    """Path of this process's seeded copy of the example library."""
    global _example_path
    with _example_lock:
        if _example_path is None:
            path = Path(tempfile.gettempdir()) / f'arbiter-example-{os.getpid()}.db'
            for suffix in ('', '-wal', '-shm'):
                Path(str(path) + suffix).unlink(missing_ok=True)
            # Set before seeding: the seeder writes through connect(), which
            # re-enters here (RLock) and must get this same path back.
            _example_path = path
            from . import example
            example.seed()
...
def connect():
    ...
    if conn is None:
        if using_example():
            path = _example_db()
            conn = getattr(_local, 'conn', None)   # the seeder may have opened it
            if conn is not None:
                return conn
        else:
            config.ensure_dirs()
...
def init_database():
    """Create YOUR (empty) library at config.DB_PATH. Refuses to touch an existing one."""
    path = Path(config.DB_PATH)
    if path.exists():
        raise FileExistsError(f'{path} already exists, leaving it alone')
    close()
