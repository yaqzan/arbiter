...
Load-bearing decisions behind Arbiter's contest/scoring model. Port of a ranking
engine from another (private) project, measured against a real corpus there.
...
`ranking.load_observations` filters on it. "Is this anime better than that crime
drama?" is not a question this app asks.
